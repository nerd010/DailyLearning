...
39. 在给自定义`View` 的`xib` 文件命名时不要用控制器去掉`Controller`后的名字，因为控制器在加载时会先匹配去掉`Controller`后的名字的`xib`文件，如果没有再去找有没有全名的`xib`文件，如果没有才会创建一个空的控制器，在找`xib`文件时它，如果控制器发现有去掉`Controller`后名字的`xib`文件，但不是给自己用的时，就会崩掉。